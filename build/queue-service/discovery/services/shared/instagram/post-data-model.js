...
const dataModelCommentsList = (json) =>
    json.edges.length === 0
        ? []
        : json.edges.map(dataModelComment)
...
const dataModelSponsorsList = (json) =>
    json.edge_media_to_sponsor_user.edges.length === 0
        ? []
        : json.edge_media_to_sponsor_user.edges.map(item => ({
...
const postDataModel = (json) => {
    ...
    try {
        return {
            ...
            commentsCount: commentsJson.count || 0,
            likesCount: data.edge_media_preview_like ? data.edge_media_preview_like.count : 0,
